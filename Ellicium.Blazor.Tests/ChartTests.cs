...
public class ChartTests
{
    ...
    public async Task Chart_Tooltip_Performance()
    {
        ...
        ctx.JSInterop.Setup<Rect>("Ellicium.createChart", _ => true).SetResult(new Rect {Left = 0, Top = 0, Width = 200, Height = 200});

        var seriesData = Enumerable.Range(0, 5000).Select(i => new Point { X = i, Y = i });
        var chart = ctx.RenderComponent<ElliciumChart>(chartParameters =>
            chartParameters
                .AddChildContent<ElliciumLineSeries<Point>>(seriesParameters =>
                    seriesParameters
                        .Add(p => p.CategoryProperty, nameof(Point.X))
                        .Add(p => p.ValueProperty, nameof(Point.Y))
                        .Add(p => p.Data, seriesData))
                .AddChildContent<ElliciumCategoryAxis>(axisParameters =>
                    axisParameters
                        .Add(p => p.Step, 100)
                        .Add(p => p.Formatter, x =>
                        {
