...
window.Ellicium = {
  ...
  addContextMenu: function (id, ref) {
     ...
     if (el) {
        var handler = function (e) {
            e.stopPropagation();
            e.preventDefault();
            ref.invokeMethodAsync('ElliciumComponent.RaiseContextMenu',
                {
                    ClientX: e.clientX,
                    ClientY: e.clientY,
                    ScreenX: e.screenX,
        ...
        Ellicium[id + 'contextmenu'] = handler;
  ...
  addMouseEnter: function (id, ref) {
     ...
     if (el) {
        var handler = function (e) {
            ref.invokeMethodAsync('ElliciumComponent.RaiseMouseEnter');
        };
        Ellicium[id + 'mouseenter'] = handler;
  ...
  addMouseLeave: function (id, ref) {
     ...
     if (el) {
        var handler = function (e) {
            ref.invokeMethodAsync('ElliciumComponent.RaiseMouseLeave');;
        };
        Ellicium[id + 'mouseleave'] = handler;
        el.addEventListener('mouseleave', handler, false);
     }
  },
  removeContextMenu: function (id) {
      var el = document.getElementById(id);
      if (el && Ellicium[id + 'contextmenu']) {
          el.removeEventListener('contextmenu', Ellicium[id + 'contextmenu']);
      }
  },
  removeMouseEnter: function (id) {
      var el = document.getElementById(id);
      if (el && Ellicium[id + 'mouseenter']) {
          el.removeEventListener('mouseenter', Ellicium[id + 'mouseenter']);
      }
  },
  removeMouseLeave: function (id) {
      var el = document.getElementById(id);
      if (el && Ellicium[id + 'mouseleave']) {
          el.removeEventListener('mouseleave', Ellicium[id + 'mouseleave']);
      }
  },
  ...
  createMap: function (wrapper, ref, id, apiKey, zoom, center, markers, options, fitBoundsToMarkersOnUpdate) {
    var api = function () {
      ...
      return new Promise(function (resolve, reject) {
        ...
        Ellicium.loadGoogleMaps(defaultView, apiKey, resolve, reject);
      });
    };

    api().then(function (google) {
      Ellicium[id] = ref;
      Ellicium[id].google = google;

      Ellicium[id].instance = new google.maps.Map(wrapper, {
        center: center,
        zoom: zoom
      });

      Ellicium[id].instance.addListener('click', function (e) {
        Ellicium[id].invokeMethodAsync('ElliciumGoogleMap.OnMapClick', {
          Position: {Lat: e.latLng.lat(), Lng: e.latLng.lng()}
        });
      });

      Ellicium.updateMap(id, zoom, center, markers, options, fitBoundsToMarkersOnUpdate);
    });
  },
  updateMap: function (id, zoom, center, markers, options, fitBoundsToMarkersOnUpdate) {
    ...
    api().then(function (google) {
        ...
        if (Ellicium[id] && Ellicium[id].instance) {
            if (Ellicium[id].instance.markers && Ellicium[id].instance.markers.length) {
                for (var i = 0; i < Ellicium[id].instance.markers.length; i++) {
                    Ellicium[id].instance.markers[i].setMap(null);
                }
            }

            if (markers) {
                Ellicium[id].instance.markers = [];
                ...
                markers.forEach(function (m) {
                    ...
                    marker.addListener('click', function (e) {
                        Ellicium[id].invokeMethodAsync('ElliciumGoogleMap.OnMarkerClick', {
                            Title: marker.title,
                            Label: marker.label,
                    ...
                    marker.setMap(Ellicium[id].instance);

                    Ellicium[id].instance.markers.push(marker);
            ...
            if (zoom) {
                Ellicium[id].instance.setZoom(zoom);
                }

            if (center) {
                Ellicium[id].instance.setCenter(center);
            }

            if (options) {
                Ellicium[id].instance.setOptions(options);
            }

            if (markers && fitBoundsToMarkersOnUpdate) {
                Ellicium[id].instance.fitBounds(markerBounds);
            }
        }
    });
  },
  destroyMap: function (id) {
    if (Ellicium[id].instance) {
      delete Ellicium[id].instance;
    }
  },
 focusSecurityCode: function (el) {
  ...
  destroySecurityCode: function (id, el) {
    if (!Ellicium[id]) return;

    var inputs = el.getElementsByTagName('input');

    if (Ellicium[id].keyPress && Ellicium[id].paste) {
        for (var i = 0; i < inputs.length; i++) {
            inputs[i].removeEventListener('keypress', Ellicium[id].keyPress);
            inputs[i].removeEventListener('paste', Ellicium[id].paste);
        }
        delete Ellicium[id].keyPress;
        delete Ellicium[id].paste;
    }
  ...
  createSecurityCode: function (id, ref, el, isNumber) {
      ...
      Ellicium[id].paste = function (e) {
          if (e.clipboardData) {
              ...
              if (value) {
                  ...
                  ref.invokeMethodAsync('ElliciumSecurityCode.OnValueChange', code);
      ...
      Ellicium[id].keyPress = function (e) {
          ...
          ref.invokeMethodAsync('ElliciumSecurityCode.OnValueChange', value);
      ...
      for (var i = 0; i < inputs.length; i++) {
          inputs[i].addEventListener('keypress', Ellicium[id].keyPress);
          inputs[i].addEventListener('paste', Ellicium[id].paste);
      }
  },
  createSlider: function (
    ...
    Ellicium[id] = {};
    Ellicium[id].mouseMoveHandler = function (e) {
    ...
    Ellicium[id].mouseDownHandler = function (e) {
    ...
    Ellicium[id].mouseUpHandler = function (e) {
      slider.canChange = false;
    };

    document.addEventListener('mousemove', Ellicium[id].mouseMoveHandler);
    document.addEventListener('touchmove', Ellicium[id].mouseMoveHandler, {
      passive: false, capture: true
    });

    document.addEventListener('mouseup', Ellicium[id].mouseUpHandler);
    document.addEventListener('touchend', Ellicium[id].mouseUpHandler, {
      passive: true
    });

    parent.addEventListener('mousedown', Ellicium[id].mouseDownHandler);
    parent.addEventListener('touchstart', Ellicium[id].mouseDownHandler, {
      passive: true
    });
  },
  destroySlider: function (id, parent) {
    ...
    if (Ellicium[id].mouseMoveHandler) {
      document.removeEventListener('mousemove', Ellicium[id].mouseMoveHandler);
      document.removeEventListener('touchmove', Ellicium[id].mouseMoveHandler);
      delete Ellicium[id].mouseMoveHandler;
    }
    if (Ellicium[id].mouseUpHandler) {
      document.removeEventListener('mouseup', Ellicium[id].mouseUpHandler);
      document.removeEventListener('touchend', Ellicium[id].mouseUpHandler);
      delete Ellicium[id].mouseUpHandler;
    }
    if (Ellicium[id].mouseDownHandler) {
      parent.removeEventListener('mousedown', Ellicium[id].mouseDownHandler);
      parent.removeEventListener('touchstart', Ellicium[id].mouseDownHandler);
      delete Ellicium[id].mouseDownHandler;
    }
  ...
  focusListItem: function (input, ul, isDown, startIndex) {
    ...
    if (
      ...
      childNodes[ul.nextSelectedIndex].classList.add('rz-state-highlight');
      Ellicium.scrollIntoViewIfNeeded(childNodes[ul.nextSelectedIndex]);
    }
        ...
        if (
            ...
            if (!cell.classList.contains('rz-state-focused')) {
                ...
                if (!isVirtual && table.parentElement.scrollWidth > table.parentElement.clientWidth) {
                    Ellicium.scrollIntoViewIfNeeded(cell);
                }
            }
        }
        ...
        if (table.nextSelectedIndex >= 0 &&
            ...
            if (!row.classList.contains('rz-state-focused')) {
                ...
                if (!isVirtual && table.parentElement.scrollHeight > table.parentElement.clientHeight) {
                    Ellicium.scrollIntoViewIfNeeded(row);
                }
            }
        }
    } 
  ...
  uploadInputChange: function (e, url, auto, multiple, clear, parameterName) {
      if (auto) {
          Ellicium.upload(e.target, url, multiple, clear, parameterName);
          e.target.value = '';
      } else {
          Ellicium.uploadChange(e.target);
      }
  },
  uploads: function (uploadComponent, id) {
    if (!Ellicium.uploadComponents) {
      Ellicium.uploadComponents = {};
    }
    Ellicium.uploadComponents[id] = uploadComponent;
  },
  uploadChange: function (fileInput) {
    ...
    var uploadComponent =
      Ellicium.uploadComponents && Ellicium.uploadComponents[fileInput.id];
    if (uploadComponent) {
      ...
      uploadComponent.invokeMethodAsync('ElliciumUpload.OnChange', files);
    }
  ...
  removeFileFromUpload: function (fileInput, name) {
    var uploadComponent = Ellicium.uploadComponents && Ellicium.uploadComponents[fileInput.id];
      ...
      if (xhr.readyState === XMLHttpRequest.DONE) {
        ...
        if (uploadComponent) {
          if (status === 0 || (status >= 200 && status < 400)) {
            uploadComponent.invokeMethodAsync(
              'ElliciumUpload.OnComplete',
                xhr.responseText,
                cancelled
            );
          } else {
            uploadComponent.invokeMethodAsync(
              'ElliciumUpload.OnError',
              xhr.responseText
            );
          }
        }
  ...
  getCulture: function () {
    var cultureCookie = Ellicium.getCookie('.AspNetCore.Culture');
  ...
  openContextMenu: function (x,y,id, instance, callback) {
    Ellicium.closePopup(id);

    Ellicium.openPopup(null, id, false, null, x, y, instance, callback);
  ...
  openTooltip: function (target, id, delay, duration, position, closeTooltipOnDocumentClick, instance, callback) {
    Ellicium.closeTooltip(id);

    if (delay) {
        Ellicium[id + 'delay'] = setTimeout(Ellicium.openPopup, delay, target, id, false, position, null, null, instance, callback, closeTooltipOnDocumentClick);
    } else {
        Ellicium.openPopup(target, id, false, position, null, null, instance, callback, closeTooltipOnDocumentClick);
    }

    if (duration) {
      Ellicium[id + 'duration'] = setTimeout(Ellicium.closePopup, duration, id, instance, callback);
    }
  },
  closeTooltip(id) {
    Ellicium.closePopup(id);

    if (Ellicium[id + 'delay']) {
        clearTimeout(Ellicium[id + 'delay']);
    }

    if (Ellicium[id + 'duration']) {
        clearTimeout(Ellicium[id + 'duration']);
    }
  },
  ...
  createDatePicker(el, popupId) {
      var toggle = function (e) {
          Ellicium.togglePopup(e.currentTarget.parentNode, popupId, false, null, null, true, false);
      };
      var button = el.querySelector('.rz-datepicker-trigger');
      if (button) {
  ...
  openPopup: function (parent, id, syncWidth, position, x, y, instance, callback, closeOnDocumentClick = true, autoFocusFirstElement = false) {
    ...
    Ellicium.activeElement = document.activeElement;
    ...
    if (window.chrome) {
        ...
        if (closestFrozenCell) {
            Ellicium[id + 'FZL'] = { cell: closestFrozenCell, left: closestFrozenCell.style.left };
    ...
    Ellicium[id] = function (e) {
        var lastPopup = Ellicium.popups && Ellicium.popups[Ellicium.popups.length - 1];
        ...
        if (!/Android/i.test(navigator.userAgent) &&
            ...
            Ellicium.closePopup(currentPopup.id, currentPopup.instance, currentPopup.callback, e);
            return;
        }
        ...
        if (closestLink && closestLink.closest && closestLink.closest('a')) {
            if (Ellicium.closeAllPopups) {
                Ellicium.closeAllPopups();
            }
        }
        if (parent) {
    ...
    Ellicium.popups.push({ id, instance, callback });

    document.body.appendChild(popup);
    document.removeEventListener('mousedown', Ellicium[id]);
    document.addEventListener('mousedown', Ellicium[id]);
    window.removeEventListener('resize', Ellicium[id]);
    window.addEventListener('resize', Ellicium[id]);

    var p = parent;
    while (p && p != document.body) {
        if (p.scrollWidth > p.clientWidth || p.scrollHeight > p.clientHeight) {
            p.removeEventListener('scroll', Ellicium.closeAllPopups);
            p.addEventListener('scroll', Ellicium.closeAllPopups);
        }
        p = p.parentElement;
    }

    if (!parent) {
        document.removeEventListener('contextmenu', Ellicium[id]);
        document.addEventListener('contextmenu', Ellicium[id]);
    }

    if (autoFocusFirstElement) {
        setTimeout(function () {
            popup.removeEventListener('keydown', Ellicium.focusTrap);
            popup.addEventListener('keydown', Ellicium.focusTrap);

            var focusable = Ellicium.getFocusableElements(popup);
  ...
  closeAllPopups: function (e, id) {
    if (!Ellicium.popups) return;
    var el = e && e.target || id && documentElement.getElementById(id);
    for (var i = 0; i < Ellicium.popups.length; i++) {
        var p = Ellicium.popups[i];
        ...
        Ellicium.closePopup(p.id, p.instance, p.callback, e);
    }
    Ellicium.popups = [];
  },
  closePopup: function (id, instance, callback, e) {
    ...
    if (popup.style.display == 'none') {
        var popups = Ellicium.findPopup(id);
        if (popups.length > 1) {
    ...
    if (popup) {
      ...
      if (window.chrome && Ellicium[id + 'FZL']) {
        Ellicium[id + 'FZL'].cell.style.left = Ellicium[id + 'FZL'].left;
        Ellicium[id + 'FZL'] = null;
      }
    ...
    Ellicium.popups = (Ellicium.popups || []).filter(function (obj) {
        return obj.id !== id;
    });

    if (Ellicium.activeElement && Ellicium.activeElement == document.activeElement ||
        Ellicium.activeElement && document.activeElement == document.body ||
        Ellicium.activeElement && document.activeElement &&
        ...
        setTimeout(function () {
            if (e && e.target && e.target.tabIndex != -1) {
                Ellicium.activeElement = e.target;
            }
            if (Ellicium.activeElement) {
               Ellicium.activeElement.focus();
            }
            Ellicium.activeElement = null;
        }, 100);
    }
  },
  ...
  togglePopup: function (parent, id, syncWidth, instance, callback, closeOnDocumentClick = true, autoFocusFirstElement = false) {
    ...
    if (popup.style.display == 'block') {
      Ellicium.closePopup(id, instance, callback);
    } else {
      Ellicium.openPopup(parent, id, syncWidth, null, null, null, instance, callback, closeOnDocumentClick, autoFocusFirstElement);
    }
  },
  ...
  openDialog: function (options, dialogService, dialog) {
    ...
    Ellicium.dialogService = dialogService;
    if (
    ...
    setTimeout(function () {
        ...
        if (lastDialog) {
            lastDialog.removeEventListener('keydown', Ellicium.focusTrap);
            lastDialog.addEventListener('keydown', Ellicium.focusTrap);
                ...
                var dialogResize = function (e) {
                    ...
                    if (dialog.offsetWidth != e[0].target.offsetWidth || dialog.offsetHeight != e[0].target.offsetHeight) {
                        ...
                        dialog.invokeMethodAsync(
                            'ElliciumDialog.OnResize',
                            e[0].target.offsetWidth,
                            e[0].target.offsetHeight
                        );
                    }
                };
                Ellicium.dialogResizer = new ResizeObserver(dialogResize).observe(lastDialog.parentElement);
            }
            ...
            if (options.autoFocusFirstElement) {
                if (lastDialog.querySelectorAll('.rz-html-editor-content').length) {
                    ...
                    var focusable = Ellicium.getFocusableElements(lastDialog);
                    var firstFocusable = focusable[0];
                    if (firstFocusable) {
                        firstFocusable.focus();
                    }
    ...
    document.removeEventListener('keydown', Ellicium.closePopupOrDialog);
    if (options.closeDialogOnEsc) {
        document.addEventListener('keydown', Ellicium.closePopupOrDialog);
    }
  },
  closeDialog: function () {
    Ellicium.dialogResizer = null;
    document.body.classList.remove('no-scroll');
    var dialogs = document.querySelectorAll('.rz-dialog-content');
    if (dialogs.length <= 1) {
        document.removeEventListener('keydown', Ellicium.closePopupOrDialog);
        delete Ellicium.dialogService;
    }
  },
  ...
  focusTrap: function (e) {
    ...
    if (isTab) {
        var focusable = Ellicium.getFocusableElements(e.currentTarget);
        var firstFocusable = focusable[0];
  ...
  closePopupOrDialog: function (e) {
      ...
      if (isEscape && Ellicium.dialogService) {
          ...
          Ellicium.dialogService.invokeMethodAsync('DialogService.Close', null);
          ...
          if (dialogs.length <= 1) {
              ...
              if (layout) {
                  layout.removeEventListener('keydown', Ellicium.disableKeydown);
              }
          }
      }
  },
  ...
  startDrag: function (ref, instance, handler) {
    ...
    return Ellicium.clientRect(ref);
  },
  ...
  startColumnReorder: function(id) {
      ...
      Ellicium[id + 'end'] = function (e) {
          var el = document.getElementById(id + 'visual');
          if (el) {
              document.body.removeChild(el);
              Ellicium[id + 'end'] = null;
              Ellicium[id + 'move'] = null;
              var resizers = cell.parentNode.querySelectorAll('.rz-column-resizer');
              for (let i = 0; i < resizers.length; i++) {
      ...
      document.removeEventListener('click', Ellicium[id + 'end']);
      document.addEventListener('click', Ellicium[id + 'end']);

      Ellicium[id + 'move'] = function (e) {
          var el = document.getElementById(id + 'visual');
          if (el) {
      ...
      document.removeEventListener('mousemove', Ellicium[id + 'move']);
      document.addEventListener('mousemove', Ellicium[id + 'move']);
  },
  stopColumnResize: function (id, grid, columnIndex) {
    ...
    if (Ellicium[el]) {
        grid.invokeMethodAsync(
            'ElliciumGrid.OnColumnResized',
            columnIndex,
            cell.getBoundingClientRect().width
        );
        el.style.width = null;
        document.removeEventListener('mousemove', Ellicium[el].mouseMoveHandler);
        document.removeEventListener('mouseup', Ellicium[el].mouseUpHandler);
        document.removeEventListener('touchmove', Ellicium[el].touchMoveHandler)
        document.removeEventListener('touchend', Ellicium[el].mouseUpHandler);
        Ellicium[el] = null;
    }
  },
  startColumnResize: function(id, grid, columnIndex, clientX) {
      ...
      Ellicium[el] = {
          ...
          mouseMoveHandler: function (e) {
              if (Ellicium[el]) {
                  var widthFloat = (Ellicium[el].width - (Ellicium[el].clientX - e.clientX));
          ...
          touchMoveHandler: function (e) {
              if (e.targetTouches[0]) {
                  Ellicium[el].mouseMoveHandler(e.targetTouches[0]);
              }
          }
      };
      el.style.width = "100%";
      document.addEventListener('mousemove', Ellicium[el].mouseMoveHandler);
      document.addEventListener('mouseup', Ellicium[el].mouseUpHandler);
      document.addEventListener('touchmove', Ellicium[el].touchMoveHandler, { passive: true })
      document.addEventListener('touchend', Ellicium[el].mouseUpHandler, { passive: true });
  },
        ...
        Ellicium[el] = {
            ...
            mouseUpHandler: function(e) {
                if (Ellicium[el]) {
                    splitter.invokeMethodAsync(
                        'ElliciumSplitter.OnPaneResized',
            ...
            mouseMoveHandler: function(e) {
                if (Ellicium[el]) {
                    ...
                    splitter.invokeMethodAsync(
                        'ElliciumSplitter.OnPaneResizing'
                    );

                    var spacePerc = Ellicium[el].panePerc + Ellicium[el].paneNextPerc;
                    var spaceLength = Ellicium[el].paneLength + Ellicium[el].paneNextLength;

                    var length = (Ellicium[el].paneLength -
                        (Ellicium[el].clientPos - (isHOrientation ? e.clientX : e.clientY)));
                    ...
                    var perc = length / Ellicium[el].paneLength;
                    if (!isFinite(perc)) {
                        perc = 1;
                        Ellicium[el].panePerc = 0.1;
                        Ellicium[el].paneLength =isHOrientation
                            ? pane.getBoundingClientRect().width
                            : pane.getBoundingClientRect().height;
                    }

                    var newPerc =  Ellicium[el].panePerc * perc;
    ...
    resizeSplitter(id, e) {
        var el = document.getElementById(id);
        if (el && Ellicium[el]) {
            Ellicium[el].mouseMoveHandler(e);
            Ellicium[el].mouseUpHandler(e);
        }
    },
    openWaiting: function() {
        ...
        if (Ellicium.WaitingIntervalId != null) {
            clearInterval(Ellicium.WaitingIntervalId);
        }
        ...
        setTimeout(function() {
                ...
                Ellicium.WaitingIntervalId = setInterval(function() {
    ...
    closeWaiting: function() {
        ...
        if (Ellicium.WaitingIntervalId != null) {
            clearInterval(Ellicium.WaitingIntervalId);
            Ellicium.WaitingIntervalId = null;
        }
    },
    toggleDictation: function (componentRef, language) {
        function start() {
            ...
            ElliciumRecognition = new SpeechRecognition();
            ElliciumRecognition.componentRef = componentRef;
            ElliciumRecognition.continuous = true;

            if (language) {
                ElliciumRecognition.lang = language;
            }

            ElliciumRecognition.onresult = function (event) {
            ...
            ElliciumRecognition.onend = function (event) {
                componentRef.invokeMethodAsync("StopRecording");
                ElliciumRecognition = null;
            };
            ElliciumRecognition.start();
        }

        if (ElliciumRecognition) {
            if (ElliciumRecognition.componentRef._id != componentRef._id) {
                ElliciumRecognition.addEventListener('end', start);
            }
            ElliciumRecognition.stop();
        } else {
            start();
        }
    }
