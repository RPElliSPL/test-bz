...
{
    /// <summary>
    ///   Base class of Ellicium Blazor components.
    /// </summary>
    public class ElliciumComponent : ComponentBase, IDisposable
    {
        /// <summary>
        ...
        /// <summary>
        /// Gets or sets a value indicating whether this <see cref="ElliciumComponent"/> is visible. Invisible components are not rendered.
        /// </summary>
        ...
        private DotNetObjectReference<ElliciumComponent> reference;

        /// <summary>
        /// Gets the reference for the current component.
        /// </summary>
        /// <value>The reference.</value>
        protected DotNetObjectReference<ElliciumComponent> Reference
        {
            get
            {
                if (reference == null)
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            ...
            if (firstRender || visibleChanged)
            {
                ...
                if (Visible)
                {
                    ...
                    {
                        await JSRuntime.InvokeVoidAsync("Ellicium.addContextMenu", GetId(), Reference);
                    }

                    if (MouseEnter.HasDelegate)
                    {
                        await JSRuntime.InvokeVoidAsync("Ellicium.addMouseEnter", GetId(), Reference);
                    }

                    if (MouseLeave.HasDelegate)
                    {
                        await JSRuntime.InvokeVoidAsync("Ellicium.addMouseLeave", GetId(), Reference);
                    }
                }
            }
        }
        ...
        [JSInvokable("ElliciumComponent.RaiseContextMenu")]
        ...
        /// </summary>
        [JSInvokable("ElliciumComponent.RaiseMouseEnter")]
        ...
        /// </summary>
        [JSInvokable("ElliciumComponent.RaiseMouseLeave")]
        ...
        public virtual void Dispose()
        {
            ...
            if (IsJSRuntimeAvailable)
            {
                if (ContextMenu.HasDelegate)
                {
                    JSRuntime.InvokeVoidAsync("Ellicium.removeContextMenu", UniqueID);
                }

                if (MouseEnter.HasDelegate)
                {
                    JSRuntime.InvokeVoidAsync("Ellicium.removeMouseEnter", UniqueID);
                }

                if (MouseLeave.HasDelegate)
                {
                    JSRuntime.InvokeVoidAsync("Ellicium.removeMouseLeave", UniqueID);
                }
            }
        }
