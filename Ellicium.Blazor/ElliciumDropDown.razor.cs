...
{
    /// <summary>
    /// ElliciumDropDown component.
    /// </summary>
    /// <typeparam name="TValue">The type of the value.</typeparam>
    /// <example>
    /// <code>
    /// &lt;ElliciumDropDown @bind-Value=@customerID TValue="string" Data=@customers TextProperty="CompanyName" ValueProperty="CustomerID" Change=@(args => Console.WriteLine($"Selected CustomerID: {args}")) /&gt;
    /// </code>
    /// </example>
    public partial class ElliciumDropDown<TValue> : DropDownBase<TValue>
    {
        /// <summary>
        ...
        {
            builder.OpenComponent(0, typeof(ElliciumDropDownItem<TValue>));
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            ...
            if (firstRender || visibleChanged || disabledChanged)
            {
                ...
                if (Visible)
                {
                    ...
                    if (!Disabled)
                    {
                        await JSRuntime.InvokeVoidAsync("Ellicium.preventArrows", Element);
                    }
            ...
            if (shouldReposition)
            {
                ...
                await JSRuntime.InvokeVoidAsync("Ellicium.repositionPopup", Element, PopupID);
            }
        }
        ...
        protected override async Task OnSelectItem(object item, bool isFromKey = false)
        {
            if (!ReadOnly)
            {
                ...
                {
                    await JSRuntime.InvokeAsync<string>("Ellicium.setInputValue", search, string.Empty);
                    searchText = null;
