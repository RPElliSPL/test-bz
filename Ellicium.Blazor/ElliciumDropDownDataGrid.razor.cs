...
{
    /// <summary>
    /// ElliciumDropDownDataGrid component.
    /// </summary>
    /// <typeparam name="TValue">The type of the t value.</typeparam>
    /// <example>
    /// <code>
    /// &lt;ElliciumDropDownDataGrid @bind-Value=@customerID TValue="string" Data=@customers TextProperty="CompanyName" ValueProperty="CustomerID" Change=@(args => Console.WriteLine($"Selected CustomerID: {args}")) /&gt;
    /// </code>
    /// </example>
    public partial class ElliciumDropDownDataGrid<TValue> : DropDownBase<TValue>
    {
        /// <summary>
...
#endif
            await JSRuntime.InvokeVoidAsync(OpenOnFocus ? "Ellicium.openPopup" : "Ellicium.togglePopup", Element, PopupID, true);

            if (FocusFilterOnPopup)
            {
                await JSRuntime.InvokeVoidAsync("Ellicium.focusElement", isFilter ? UniqueID : SearchID);
            }

            if (list != null)
            {
                await JSRuntime.InvokeVoidAsync("Ellicium.selectListItem", search, list, selectedIndex);
            }
        }
        ...
        /// <summary>
        /// Gets or sets a value indicating whether this <see cref="ElliciumDropDownDataGrid{TValue}"/> is responsive.
        /// </summary>
        ...
        protected override Task OnAfterRenderAsync(bool firstRender)
        {
            ...
            {          
                ...
                {
                    LoadData.InvokeAsync(new Ellicium.LoadDataArgs() { Skip = 0, Top = PageSize, Filter = searchText });
                }
        ...
        protected override async Task OnDataChanged()
        {
            ...
            {
                searchText = null;
                await OnLoadData(new Ellicium.LoadDataArgs() { Skip = 0, Top = PageSize });
            }
        }
        ...
        public override async Task SetParametersAsync(ParameterView parameters)
        {
            ...
            {
                await OnLoadData(new Ellicium.LoadDataArgs() { Skip = skip, Top = PageSize });
            }
        }
        ...
        private bool IsColumnFilterPropertyTypeString(ElliciumDataGridColumn<object> column)
        {
        ...
        async Task OnLoadData(LoadDataArgs args)
        {
            ...
            {
                await LoadData.InvokeAsync(new Ellicium.LoadDataArgs() { Skip = skip, Top = args.Top, OrderBy = args.OrderBy, Filter = searchText });
            }
        }
        ...
        protected override async Task HandleKeyPress(KeyboardEventArgs args, bool isFilter, bool? shouldSelectOnChange = null)
        {
            ...
            if (!args.AltKey && (key == "ArrowDown" || key == "ArrowUp"))
            {
                ...
                try
                {
                    ...
                    if (shouldChange)
                    {
                        ...
                        await JSRuntime.InvokeAsync<int[]>("Ellicium.focusTableRow", grid.GridId(), key, selectedIndex - 1, null);
                        await grid.OnRowSelect(items[selectedIndex], false);
                    }

                    if (!Multiple)
                    {
                        var popupOpened = await JSRuntime.InvokeAsync<bool>("Ellicium.popupOpened", PopupID);
            ...
            else if (key == "Enter" || key == "NumpadEnter")
            {
                ...
                else
                {
                    ...
                    if (!Multiple)
                    {
                        await JSRuntime.InvokeVoidAsync("Ellicium.closePopup", PopupID);
                    }
                }
            }
        ...
        async Task RefreshAfterFilter()
        {
            ...
            await JSRuntime.InvokeAsync<string>("Ellicium.repositionPopup", Element, PopupID);
        }
        ...
        async Task OnRowSelect(object item)
        {
            ...
            await JSRuntime.InvokeVoidAsync("Ellicium.focusElement", UniqueID);
        ...
        public override void Dispose()
        {
            ...
            {
                JSRuntime.InvokeVoidAsync("Ellicium.destroyPopup", PopupID);
            }
        }
        ...
        /// </summary>
        public ElliciumDataGrid<object> DataGrid
        {
            get
            {
                return grid;
