...
{
    /// <summary>
    /// ElliciumMediaQuery fires its <see cref="Change" /> event when the media query specified via <see cref="Query" /> matches or not.
    /// </summary>
    /// <example>
    /// &lt; ElliciumMediaQuery Query="(max-width: 768px)" Change=@OnChange /&gt;
    /// @code {
    ...
    ///}
    /// </example>
    public class ElliciumMediaQuery : ComponentBase, IDisposable
    {
        ...
        bool initialized;
        private DotNetObjectReference<ElliciumMediaQuery> reference;
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender)
            {
                ...
                try
                {
                    var matches = await JSRuntime.InvokeAsync<bool>("Ellicium.mediaQuery", Query, Reference);
        ...
        public void Dispose()
        {
            if (initialized)
            {
                try
                {
                    JSRuntime.InvokeVoidAsync("Ellicium.mediaQuery", Reference);
                }
                catch
                {
                    //
