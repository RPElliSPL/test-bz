...
{
    /// <summary>
    /// ElliciumColorPicker component.
    /// </summary>
    /// <example>
    /// <code>
    /// &lt;ElliciumColorPicker @bind-Value=@color Change=@(args => Console.WriteLine($"Selected color: {args}")) /&gt;
    /// </code>
    /// </example>
    public partial class ElliciumColorPicker : FormComponent<string>
    {
        ...
        async Task OnHueKeyPress(KeyboardEventArgs args)
        {
            ...
            if (key == "ArrowLeft" || key == "ArrowRight")
            {
                ...
                if (lastHueRect == null)
                {
                    lastHueRect = await JSRuntime.InvokeAsync<Rect>("Ellicium.clientRect", (GetId() + "hue"));
                }
        ...
        async Task OnAlphaKeyPress(KeyboardEventArgs args)
        {
            ...
            if (key == "ArrowLeft" || key == "ArrowRight")
            {
                ...
                {
                    lastAlphaRect = await JSRuntime.InvokeAsync<Rect>("Ellicium.clientRect", (GetId() + "alpha"));
                }
        ...
        async Task OnHslKeyPress(KeyboardEventArgs args)
        {
            ...
            if (lastHslRect == null)
            {
                lastHslRect = await JSRuntime.InvokeAsync<Rect>("Ellicium.clientRect", (GetId() + "hsl"));
            }
        ...
        {
            await Popup.CloseAsync();
            await JSRuntime.InvokeVoidAsync("Ellicium.focusElement", GetId());
        }
    }
}
