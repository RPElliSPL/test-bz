...
namespace Ellicium
{
    ...
    public class DropDownBase<T> : DataBoundFormComponent<T>
    {
        ...
        private async ValueTask<Microsoft.AspNetCore.Components.Web.Virtualization.ItemsProviderResult<object>> LoadItems(Microsoft.AspNetCore.Components.Web.Virtualization.ItemsProviderRequest request)
        {
            ...
            {
                await LoadData.InvokeAsync(new Ellicium.LoadDataArgs() { Skip = request.StartIndex, Top = top, Filter = searchText });
            }
        ...
        protected async System.Threading.Tasks.Task ClearAll()
        {
            ...
            await JSRuntime.InvokeAsync<string>("Ellicium.setInputValue", search, "");
        ...
        public object GetItemOrValueFromProperty(object item, string property)
        {
            if (item != null)
            {
                ...
                if (enumValue != null)
                {
                    return Ellicium.Blazor.EnumExtensions.GetDisplayDescription(enumValue);
                }
            }
        ...
        protected string OpenPopupScript()
        {
            ...
            return $"Ellicium.togglePopup(this.parentNode, '{PopupID}', true);Ellicium.focusElement('{SearchID}');";
        }
        ...
        protected string OpenPopupScriptFromParent()
        {
            ...
            return $"Ellicium.togglePopup(this, '{PopupID}', true);Ellicium.focusElement('{SearchID}');";
        }
        ...
        protected virtual async System.Threading.Tasks.Task OpenPopup(string key = "ArrowDown", bool isFilter = false, bool isFromClick = false)
        {
            ...
            await JSRuntime.InvokeVoidAsync("Ellicium.togglePopup", Element, PopupID, true);
        ...
        protected virtual async System.Threading.Tasks.Task HandleKeyPress(Microsoft.AspNetCore.Components.Web.KeyboardEventArgs args, bool isFilter = false, bool? shouldSelectOnChange = null)
        {
            ...
            if (!args.AltKey && (key == "ArrowDown" || key == "ArrowLeft" || key == "ArrowUp" || key == "ArrowRight"))
            {
                ...
                try
                {
                    selectedIndex = await JSRuntime.InvokeAsync<int>("Ellicium.focusListItem", search, list, key == "ArrowDown" || key == "ArrowRight", selectedIndex);
            ...
            else if (key == "Enter" || key == "NumpadEnter")
            {
                ...
                {
                    await JSRuntime.InvokeAsync<string>("Ellicium.setInputValue", search, $"{searchText}".Trim());
                    await OnSelectItem(items.ElementAt(selectedIndex), true);
                }
            ...
            else 
            {
                ...
                if (itemToSelect != null)
                {
                    ...
                    if (result != null)
                    {
                        ...
                        await JSRuntime.InvokeVoidAsync("Ellicium.selectListItem", list, list, result.Index);
                    }                    
                }
        ...
        internal virtual async System.Threading.Tasks.Task<LoadDataArgs> GetLoadDataArgs()
        {
            ...
            if (AllowVirtualization)
            {
                return await Task.FromResult(new Ellicium.LoadDataArgs() { Skip = 0, Top = PageSize, Filter = searchText });
            }
            else
            {
                return await Task.FromResult(new Ellicium.LoadDataArgs() { Filter = searchText });
            }
#else
            return await Task.FromResult(new Ellicium.LoadDataArgs() { Filter = searchText });
#endif
